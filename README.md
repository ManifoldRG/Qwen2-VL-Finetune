...
### Evaluate UITARS on recorded episodes (Mind2Web-style outputs)

If you have recorded episodes with the following structure:

- `<episode_dir>/screenshots/*.png`
- `<episode_dir>/trajectory.json` (list of step objects, including fields like `step_instruction`, `confirmed_task`, and `screenshot`)

You can run the UITARS evaluation over a single episode or an entire run directory:

```bash
# Single episode
python -m eval.uitars15_runner \
  --episode_dir /abs/path/to/outputs/run_XXXXXXXX/<episode_id> \
  --model ByteDance-Seed/UI-TARS-1.5-7B \
  --output_jsonl /abs/path/to/results.jsonl

# Entire run directory (evaluates all episode subfolders with screenshots/ + trajectory.json)
python -m eval.uitars15_runner \
  --run_dir /abs/path/to/outputs/run_XXXXXXXX \
  --model ByteDance-Seed/UI-TARS-1.5-7B \
  --output_jsonl /abs/path/to/results.jsonl
```

Tip: If your episodes were generated by `WebDomainRandomizer`, point `--run_dir` to its `outputs/run_*` folder (e.g., `/Users/you/Repos/personal/manifold/WebDomainRandomizer/outputs/run_20251104_233524`).
